...
After the services have access to the "user" entity, they check the projects of the user (including projects of the user's groups) compared to the requested projects.

The majority of the logic of the authorization rules are shared between all the services in [/packages/api-base/src/auth.ts](https://github.com/PabloSzx/learner-model-gql/blob/main/packages/api-base/src/auth.ts),
many of them defined as Lazy [JavaScript Promises](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise), where based on where they are [awaited](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/await),
they are applied in the respective resolver.

An example of the mention authorization checkers is the `expectAdmin` rule, which is defined as:
...
The `expectUser` asserts that the user exists and is authenticated, and the `expectAdmin` re-uses the `expectUser` rule, and asserts that the user has the role `ADMIN`.
